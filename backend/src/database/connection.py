...
def _mongodb_uri() -> str:
    # Read lazily (not at module-import time) so this always sees the real
    # value regardless of when/where load_dotenv() was called relative to
    # this import -- module-level os.getenv() here previously froze in the
    # localhost default whenever database.py was imported before .env loaded.
    return os.getenv("MONGODB_URI", "mongodb://localhost:27017/")


def _mongodb_db_name() -> str:
    return os.getenv("MONGODB_DB", "agrismart_ai")


def is_configured() -> bool:
    """True only when MONGODB_URI is explicitly set.

    Persistence is opt-in: without this, api_server.py never attempts a
    Mongo connection, so requests don't hang waiting on
    serverSelectionTimeoutMS when no database is running.
    """
...
def get_db() -> Database:
    ...
    global _db
    if _db is None:
        _db = get_client()[_mongodb_db_name()]
    return _db
